...
#'
#' @description This function computes and compares ontogenetic vectors among
#'   species in a tree.
...
#' @param node the number identifying the most recent common ancestor to all the
#'   species the user wants ontogenetic vectors be computed.
#' @param Y multivariate trait values at tips.
#' @param type specifies weather to perform the analysis on phenotypic
#'   (\code{"phenotypes"}) or rate (\code{"rates"}) vectors.
#' @param select.axes if \code{"yes"}, \code{Y} variables are individually
...
#' @export
#' @details The \code{angle.matrix} function takes as objects a phylogenetic
...
#' @return \enumerate{\item \strong{$regression.matrix} a 'list' including
